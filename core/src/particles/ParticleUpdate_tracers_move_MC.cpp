...
class ParticleUpdate_tracers_move_MC : public ParticleUpdate {
  ...
  void update( UserData& U, ScalarSimulationData& scalar_data) 
  {
      ...
      KOKKOS_LAMBDA( const ForeachParticle::ParticleIndex& iPart )
    {
      ...
      real_t p_out = (fxm + fxp + fym + fyp + fzm + fzp);
      ...
      for (int k = 0; k < nfaces; ++k) {
        DYABLO_ASSERT_KOKKOS_DEBUG(faces[k].mass >= (real_t)0,
                                   "face mass must be non-negative");

        real_t pk = (sum > (real_t)0) ? faces[k].mass / sum : (real_t)0;
        if ((real_t)r2 < pk) { chosen = k; break; }
        r2 -= pk;
      }
      DYABLO_ASSERT_KOKKOS_DEBUG(chosen >= 0,
                                 "roulette selection failed (chosen<0)");
